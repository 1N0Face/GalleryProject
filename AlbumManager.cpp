...
//This function shows the picture with the path
void AlbumManager::showPicture()
{
	...
	std::string paint = "C:\\Windows\\system32\\mspaint.exe \"" + p + "\"";
	std::string irfanView = "C:\\Program Files\\IrfanView\\i_view64.exe \"" + p + "\"";
	std::string prog = getInputFromConsole("Enter P for paint and I for IrfanView: ");
	SetConsoleCtrlHandler(HandlerRoutine, TRUE);
	bool flag = false;
	switch (prog[0]){
	case 'P':
		flag = CreateProcessW(NULL, A2W_EX(paint.c_str(), paint.length()), NULL, NULL, FALSE, 0, NULL, NULL, &info, &processInfo);
		break;
	case 'I':
		flag = CreateProcessW(NULL, A2W_EX(irfanView.c_str(), irfanView.length()), NULL, NULL, FALSE, 0, NULL, NULL, &info, &processInfo);
		break;
	default:
		throw MyException("Error: There is no option with this input");
	}
