	...
	if (doesUserExists(user.getId())) {
		...
		for (auto iter = m_users.begin(); iter != m_users.end(); ++iter) {
			if (*iter == user) {
				...
				for (auto iter2 = m_albums.begin(); iter2 != m_albums.end(); iter2++)
				{
					iter2->untagUserInAlbum(user.getId());
					if (iter2->getOwnerId() == user.getId())
					{
						this->deleteAlbum(iter2->getName(), user.getId());
						iter2 = m_albums.begin();
					}
				}
				return;
			}
